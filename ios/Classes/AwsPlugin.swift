  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
   ...
   private func decideRegion(_ region: String) -> AWSRegionType {
    ...
    private func uploadFile(result: @escaping FlutterResult, args: Any?) {
        ...
        let awsFolder = argsMap["awsFolder"]
        let key = ((argsMap["awsFolder"] as! String) + "/" + (argsMap["fileNameWithExt"] as! String))
        let expireSecond = 30
        let s3BucketName = argsMap["bucketName"]
        let region = argsMap["region"]
        let accessKey = argsMap["AWSAccess"]
        let secretKey = argsMap["AWSSecret"]
        ...
        let configuration = AWSServiceConfiguration.init(
            region: decideRegion(region as! String),
            credentialsProvider: credentialsProvider
        )
        AWSServiceManager.default().defaultServiceConfiguration = configuration

        guard let imageURL: URL = URL(string: filePath as! String)
        else {
            return
        }
        ...
        transferUtility.uploadFile(
            ...
            expression: expression, completionHandler: completionHandler).continueWith(block: { task in
                print(task)
            })
      }
...
//            let fileNameWithExt = argsMap["fileNameWithExt"],
//            let bucketName = argsMap["bucketName"],
//            let region = argsMap["region"],
//            let access = argsMap["AWSAccess"],
//            let secret = argsMap["AWSSecret"] {
//
//            let convertedRegion = decideRegion(region as! String)
//            ///
//            let credentialsProvider = AWSStaticCredentialsProvider(accessKey: access as! String, secretKey: secret as! String)
